...
programs.zsh = {
  ...
  enable = true; 
	...
	ani-tupi = "nix-shell --pure -p uv mpv fzf curl bash xdg-utils --run 'export PATH=/run/current-system/sw/bin:/usr/bin:/bin:$PATH; export DISPLAY=$DISPLAY; export WAYLAND_DISPLAY=$WAYLAND_DISPLAY; export XDG_RUNTIME_DIR=$XDG_RUNTIME_DIR; export DBUS_SESSION_BUS_ADDRESS=\"\"; uv run --project $HOME/ani-tupi $HOME/ani-tupi/main.py'";
