...
programs.zsh = {
  ...
  enable = true; 
	...
	ani-tupi = "nix shell nixpkgs#uv nixpkgs#mpv nixpkgs#fzf nixpkgs#curl nixpkgs#yt-dlp -c sh -c 'env -i HOME=$HOME DISPLAY=$DISPLAY WAYLAND_DISPLAY=$WAYLAND_DISPLAY XDG_RUNTIME_DIR=$XDG_RUNTIME_DIR PATH=/run/current-system/sw/bin uv run --script https://githubusercontent.com'";
