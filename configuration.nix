...
programs.zsh = {
  ...
  enable = true; 
	...
	ani-tupi = "nix-shell --pure -p uv mpv fzf curl bash xdg-utils --run 'export PATH=/run/current-system/sw/bin:/usr/bin:/bin:$PATH; export DISPLAY=$DISPLAY; export WAYLAND_DISPLAY=$WAYLAND_DISPLAY; export XDG_RUNTIME_DIR=$XDG_RUNTIME_DIR; export LD_LIBRARY_PATH=/run/opengl-driver/lib:/run/opengl-driver-32/lib:$LD_LIBRARY_PATH; export AMD_VULKAN_ICD=RADV; export DBUS_SESSION_BUS_ADDRESS=\"\"; export PLAYER=mpv; uv run --project $HOME/ani-tupi $HOME/ani-tupi/main.py'";
  ...
  systemd.services.teclado-led-trigger = {
    ...
    serviceConfig = {
      ...
      ExecStart = pkgs.writeScript "teclado-trigger-filtrado" ''
        ...
        # MAPA: tecla -> (LED correspondente, nome)
        MAPA_TECLAS = {
        ...
        # Estado lógico local por scancode (para não depender de d.leds())
        estado_local = {58: 0, 69: 0, 70: 0}

        # Estado local independente para re-aplicar Num após mudanças do Caps
        # (o driver/firmware do teclado mexe no LED de Num quando Caps muda)
        NUM_LAST = 0

        def monitorar_interface(dev, todos_os_devs):
            global TECLAS_PRESSIONADAS, estado_local, NUM_LAST
            try:
                for event in dev.read_loop():
                    ...
                    if data.keystate == 1 and not TECLAS_PRESSIONADAS[data.scancode]:
                        with lock_global:
                            ...
                            # toggle lógico apenas do LED alvo
                            estado_local[data.scancode] = 0 if estado_local.get(data.scancode, 0) == 1 else 1
                            novo_estado_alvo = estado_local[data.scancode]

                            # Atualiza cache do Num quando a tecla Num for pressionada
                            if data.scancode == 69:
                                NUM_LAST = novo_estado_alvo

                            # Aplica o LED alvo
                            for d in todos_os_devs:
                                try:
                                    d.set_led(led_alvo, novo_estado_alvo)
                                except:
                                    pass

                            # Correção: quando Caps muda, re-aplica o último estado do Num
                            # para neutralizar o acoplamento do firmware.
                            if data.scancode == 58:
                                for d in todos_os_devs:
                                    try:
                                        d.set_led(ecodes.LED_NUML, NUM_LAST)
                                    except:
                                        pass

                            print(f"REGRA ESTRITA: {nome_tecla} alterado para {novo_estado_alvo}")
                            sys.stdout.flush()
